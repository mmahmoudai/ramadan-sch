  ...
  return (
    <div className="space-y-6">
      ...
      {showForm && (
        <div className="border-2 border-line rounded-xl bg-card p-4">
          ...
          <form onSubmit={createChallenge} className="space-y-3">
            <input type="text" placeholder="Challenge title" value={title} onChange={(e) => setTitle(e.target.value)} required className="w-full border-2 border-line rounded-lg px-3 py-2" />
            <textarea placeholder="Description (optional)" value={description} onChange={(e) => setDescription(e.target.value)} rows={3} className="w-full border-2 border-line rounded-lg px-3 py-2" />
            ...
            <button type="submit" className="bg-accent text-white px-6 py-2 rounded-lg font-bold text-sm hover:opacity-90">Create</button>
          </form>
        </div>
      )}
      ...
      <div className="space-y-3">
        {challenges.map((c) => (
          <div key={c._id} className="border-2 border-line rounded-xl bg-card p-4">
            <div className="flex items-start justify-between">
              <div className="flex-1">
                ...
                <div className="flex gap-2 mt-2">
                  ...
                  <span className="text-xs text-gray-500">{c.progress?.length || 0} progress entries</span>
                </div>
              </div>
              <div className="flex gap-2">
                ...
                <button onClick={() => setEditingChallenge(editingChallenge === c._id ? null : c._id)} className="text-xs px-3 py-1 rounded-lg border border-line hover:bg-gray-100 font-semibold">
                  {editingChallenge === c._id ? "Cancel" : "Update Progress"}
                </button>
            ...
            {editingChallenge === c._id && (
              <div className="mt-4 p-3 bg-gray-50 rounded-lg border border-gray-200">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mb-3">
                  ...
                  <div>
                    <label className="block text-xs font-semibold mb-1">Progress (%)</label>
                    <input
                      type="number"
                      min="0"
                      max="100"
            ...
            {c.progress && c.progress.length > 0 && (
              <div className="mt-4">
                <h4 className="font-semibold text-sm mb-2">Progress History</h4>
                      ...
                      <div key={p.dateGregorian} className="bg-white rounded-lg px-3 py-2 border border-gray-100">
                        <div className="flex items-center justify-between mb-1">
                          <div className="flex items-center gap-3">
                            ...
                            {p.completed && <span className="text-xs bg-green-100 text-green-700 px-2 py-0.5 rounded font-bold">Done</span>}
                          </div>
        ...
        {challenges.length === 0 && <p className="text-gray-500 text-center py-8">No challenges yet. Create your first one!</p>}
      </div>
    </div>
  );
}
